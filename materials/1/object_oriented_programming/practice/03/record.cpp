...
Record::Record(const Record &rec) {
  ...
  for (int i = 0; i < wordLen; i++)
    this->word[i] = rec.word[i];
  this->word[wordLen] = '\0';
  for (int i = 0; i < meaningLen; i++)
    this->meaning[i] = rec.meaning[i];
  this->meaning[meaningLen] = '\0';
}
...
bool Record::compare(Record &rec1, Record &rec2) {
  ...
  if (firstWordLen > secondWordLen)
    return true;
  else if (firstWordLen < secondWordLen)
    return false;

  int firstMeaningLen = strlen(rec1.meaning),
      secondMeaningLen = strlen(rec2.meaning);
  for (int i = 0; i < min(firstMeaningLen, secondMeaningLen); i++)
    if (rec1.meaning[i] != rec2.meaning[i])
      if (rec1.meaning[i] > rec2.meaning[i])
        ...
        return false;
  if (firstMeaningLen > secondMeaningLen)
    return true;
  return false;
}